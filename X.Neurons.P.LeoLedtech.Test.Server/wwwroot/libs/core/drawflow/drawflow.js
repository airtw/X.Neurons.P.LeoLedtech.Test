class Drawflow {
  ...
  addNode (name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false, tooltip_in_data = [], tooltip_out_data = []) {
    if (this.useuuid) {
    ...
    for(var x = 0; x < num_in; x++) {
      ...
      // 添加註解的 tooltip
      if (tooltip_in_data.length >= 1){
        //如果有描述就新增，沒有就單純加節點資訊
        if(tooltip_in_data[x].text !== null && tooltip_in_data[x].text !== ''){
          const tooltip = document.createElement('div');
          tooltip.classList.add('tooltip');
          tooltip.innerText = `Input ${x+1} ${tooltip_in_data[x].text}`; // 你可以自定義註解文字
          input.appendChild(tooltip);
          json_inputs["input_"+(x+1)] = { 
              "connections": [],
              "tooltip": `Input ${x+1} ${tooltip_in_data[x].text}` // 儲存註解內容
          };
        }else{
          json_inputs["input_"+(x+1)] = { "connections": []};
        }
    ...
    for(var x = 0; x < num_out; x++) {
      ...
      //如果有tooltip就新增，沒有就單純加節點資訊
      if (tooltip_out_data.length >= 1){
        if(tooltip_out_data[x].type === 'success'){
          output.classList.add("success");
        }
        else if(tooltip_out_data[x].type === 'fail'){
          output.classList.add("fail");
        }
        //如果有描述就新增，沒有就單純加節點資訊
        if(tooltip_out_data[x].text !== null && tooltip_out_data[x].text !== ''){
          const tooltip = document.createElement('div');
          tooltip.classList.add('tooltip');
          tooltip.innerText = `Output ${x+1} ${tooltip_out_data[x].text}`;
          output.appendChild(tooltip);
          json_outputs["output_"+(x+1)] = { 
            "connections": [],
            "tooltip": `Output ${x+1} ${tooltip_out_data[x].text}` // 儲存註解內容
        };
        }else{
          json_outputs["output_"+(x+1)] = { "connections": []};
        }
